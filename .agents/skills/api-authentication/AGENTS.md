...
| Agente | Como usar |
|---|---|
| Claude Code | Copiar a pasta para `.claude/skills/api-authentication/` (projeto) ou `~/.claude/skills/api-authentication/` (global). Aciona pela `description` do frontmatter. |
...
Autenticação e autorização de APIs: JWT (RS256), OAuth2/OIDC, API keys e sessões. Use SEMPRE ao proteger APIs, emitir/validar tokens, implementar login, refresh, MFA ou auth serviço-a-serviço. Alinhado ao security baseline. Acionar em "autenticação", "login", "JWT", "OAuth", "OIDC", "API key", "token", "refresh token", "proteger API", "auth serviço".
